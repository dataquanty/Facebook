...
for c in cols:
    try:
        mat[c] = pd.cut(mat[c],30,labels=False)
    except:
...
for i in xrange(1,int(n_est/10)):
    ...
    train_score[i:(i+1)*10] = roc_auc_score(y_train,clf.predict(X_train))
    test_score_roc[i:(i+1)*10] = roc_auc_score(y_test, clf.predict(X_test))
...
Gradboost = GradientBoostingClassifier(n_estimators=3100, learning_rate= 0.05, max_depth= 10)
...
Bagging = BaggingClassifier(base_estimator=calibrated_clf,n_estimators=10,random_state=23,n_jobs=4)
Bagging.fit(X,Y)
